  ...
  async overview(@Req() request: AuthenticatedRequest) {
    ...
    const [
      ...
      this.prisma.project.findMany({
        ...
        include: { client: true, modules: true },
      }),
    ...
    const currentMonthRevenue = revenueForPeriod(
      financial,
      (date) =>
        date.getFullYear() === currentYear && date.getMonth() === currentMonth,
    );
    const previousMonthRevenue = revenueForPeriod(
      financial,
      (date) =>
        date.getFullYear() === previousMonthDate.getFullYear() &&
        date.getMonth() === previousMonthDate.getMonth(),
    );
    const currentYearRevenue = revenueForPeriod(
      financial,
      (date) => date.getFullYear() === currentYear,
    );
    const previousYearRevenue = revenueForPeriod(
      financial,
    ...
    const projectsWithProgress = projects.map((project) => ({
      ...project,
      progress: moduleProgress(project.modules),
    }));
    const averageProgress = projectsWithProgress.length
      ? Math.round(
          projectsWithProgress.reduce((sum, item) => sum + item.progress, 0) /
    ...
    return {
      kpis: {
        ...
        revenueMonthChange: percentageChange(
          currentMonthRevenue,
          previousMonthRevenue,
        ),
        revenueYear: currentYearRevenue,
        revenueYearChange: percentageChange(
          currentYearRevenue,
          previousYearRevenue,
        ),
        averageProgress,
...
function revenueForPeriod(
  ...
  return transactions
    .filter((item) => {
      ...
      return (
        item.status === "PAID" && item.kind === "REVENUE" && inPeriod(paidDate)
      );
    })
...
function moduleProgress(
  modules: Array<{ completed: boolean; progress: number }>,
) {
